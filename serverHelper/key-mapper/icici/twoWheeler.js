...
import { IciciTwMakeModelVariantHandler } from 'server_helper/serverSideFunctions/vehicle/twoWheeler';
import TwRtoArray from '../../db/icici/TwRtoMaster.json';
import MasterRtoArray from '../../db/kotak/master/tw_rto_master.json';
import { IciciRtoDetails } from "server_helper/serverSideFunctions/rto";

///////////////////////////////////////////////////  PREMIUM  ///////////////////////////////////////////

export let  IciciTwPremiumClientToServerMapper = (client) => {
  ...
  /////////////////////////----- VEHICLE & RTO DETAILS -----/////////////////////////
  let vehicleDetails = IciciTwMakeModelVariantHandler(client);
  ...
  let AddressLine1 = ""
  if (client.c_dob) {
    DateOfBirth = format(new Date(client.c_dob), 'yyyy/MM/dd');
  }

  if (client.c_address_line1 || client.c_address_line2) {
    AddressLine1 = `${client.c_address_line1} ${client.c_address_line2}`;
  }
  ...
  let financierDetails = {
    FinancierName: client.c_financier_name ? client.c_financier_name : '',
    BranchName: '',
    AgreementType: '',
  };
  ...
  let server_object = {
    ...
    RegistrationNumber: client.c_rollover ? client.c_registration_number ? client?.c_registration_number : "" : "NEW",
    ...
    PreviousPolicyDetails: {
      ...
      ClaimOnPreviousPolicy: client.c_claim_last_year ? true : false,
      TotalNoOfODClaims: '0',
    ...
    CustomerDetails: {
      ...
      StateCode: 65,
      CityCode: 200,
      Gender: client.c_gender ? client.c_gender.toLowerCase() === 'male' ? 'MALE' : 'FEMALE' : "",
      GSTDetails: '',
    ...
    Tenure: 0,
    TPTenure: 0,
    PACoverTenure: client.c_policy_tenure ? client.c_policy_tenure : 0,
  ...
  if (client.c_financier_name) {
    server_object = { ...server_object, ...financierDetails };
  }
...
export let IciciTwPremiumServerToClientMapper = (server) => {
  let basicOdPremium = null;
  let makeModelDetails = `${server.generalInformation.manufacturerName} ${server.generalInformation.vehicleModel}`;
  let rtoDetails = `${server.generalInformation.rtoLocation}`;

  if (server.totalLiabilityPremium && server.riskDetails.basicTP) {
    basicOdPremium = server.totalLiabilityPremium - server.riskDetails.basicTP;
  }

  const server_object = {
    s_net_premium: server.totalLiabilityPremium,
    s_total_premium: server.finalPremium,
    s_gst_amount: server.totalTax,
    s_basic_tp_premium: server.riskDetails.basicTP,
    s_basic_od_premium: basicOdPremium,
    s_make_model_details: makeModelDetails,
    s_rto_details: rtoDetails,
    s_legal_liability_pd_premium: server.riskDetails.paidDriver,
    s_pa_unnamed_passenger_premium: server.riskDetails.paCoverForUnNamedPassenger,
    s_pa_cover_owner_driver_premium: server.riskDetails.paCoverForOwnerDriver,
    s_limit_tppd_premium: server.riskDetails.tppD_Discount,
    s_ncb_percent: server.riskDetails.ncbPercentage,
    s_correlation_Id: server.correlationId,
    s_company_name: 'icici',
    s_service_type: 'two_wheeler',
    s_plan_type: 'premium',
    s_plan_name: 'ICICI Insurance',
  };
...
export const IciciTwProposalClientToServerMapper = (client) => {
  let vehicleDetails = IciciTwMakeModelVariantHandler(client);
  if (Object.keys(vehicleDetails).length === 0) {
    return errorResponse('Vehicle Details not available ICICI Database');
  }
  ...
  if (!PreviousPolicyEndDate) return errorResponse('Previous Policy date should not be null or invalid format');
  ...
  let client_object = {
    ...
    IsExtensionCountry: true,
    ExtensionCountryName: 'Bhutan, Bangladesh, Nepal, Pakistan, Maldives, SriLanka',
    IsMoreThanOneVehicle: false,
    IsLegalLiabilityToPaidDriver: client?.company_object?.s_legal_liability_pd_premium ? client?.company_object?.s_legal_liability_pd_premium : false,
    NoOfDriver: 1,
    IsLegalLiabilityToPaidEmployee: false,
    NoOfEmployee: 1,
    IsValidDrivingLicense: false,
    IsPACoverUnnamedPassenger: Number(client.c_pa_unnamed_passenger) ? true : false,
    SIPACoverUnnamedPassenger: Number(client.c_pa_unnamed_passenger) ? Number(client.c_pa_unnamed_passenger) : 0,
    TPPDLimit: client.c_limit_tppd ? Number(client.c_limit_tppd) : '0',
    ...
    CustomerDetails: {
      ...
      StateCode: 65,
      CityCode: 200,
      Gender: client.c_gender.toLowerCase() === 'male' ? 'MALE' : 'FEMALE',
      GSTDetails: {
  ...
  if (client.c_financier_name) {
    client_object = { ...client_object, ...financierDetails };
  }
...
export const IciciTwProposalServerToClientMapper = (server) => {
  let basicOdPremium = null;
  let makeModelDetails = `${server?.generalInformation?.manufacturerName} ${server?.generalInformation?.vehicleModel}`;
  let rtoDetails = `${server?.generalInformation?.rtoLocation}`;
  ...
  const server_object = {
    ...
    s_limit_tppd_premium: server.riskDetails.tppD_Discount,
    s_ncb_percent: server.riskDetails.ncbPercentage,
    s_proposal_number: server.generalInformation.proposalNumber,
    s_customer_id: server.generalInformation.customerId,
    s_correlation_Id: server.correlationId,
    s_company_name: 'icici',
    s_service_type: 'two_wheeler',
    s_plan_type: 'premium',
